...
public class GraphNodeCensor extends BaseCensor {

	private static final LoggerService logger = new LoggerService(LoggerFactory.getLogger(GraphNodeCensor.class));

	protected final AuthorizationService authService;
	protected final CensorFactory censorFactory;

	@Autowired
	public GraphNodeCensor(
	...
	public void censor(FieldSet fields, UUID userId) throws MyForbiddenException {
		...
		this.authService.authorizeForce(Permission.BrowseGraphNode);
		FieldSet graphFields = fields.extractPrefixed(this.asIndexerPrefix(GraphNode._graph));
		this.censorFactory.censor(GraphCensor.class).censor(graphFields, userId);
		FieldSet nodeFields = fields.extractPrefixed(this.asIndexerPrefix(GraphNode._node));
