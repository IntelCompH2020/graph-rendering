...
public class GraphEdgeCensor extends BaseCensor {

	private static final LoggerService logger = new LoggerService(LoggerFactory.getLogger(GraphEdgeCensor.class));

	protected final AuthorizationService authService;
	protected final CensorFactory censorFactory;

	@Autowired
	public GraphEdgeCensor(
	...
	public void censor(FieldSet fields, UUID userId) throws MyForbiddenException {
		...
		this.authService.authorizeForce(Permission.BrowseGraphEdge);
		FieldSet graphFields = fields.extractPrefixed(this.asIndexerPrefix(GraphEdge._graph));
		this.censorFactory.censor(GraphCensor.class).censor(graphFields, userId);
		FieldSet edgeFields = fields.extractPrefixed(this.asIndexerPrefix(GraphEdge._edge));
